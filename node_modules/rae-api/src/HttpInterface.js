...
class HttpInterface {
	...
	sendRequest(endpoint){
		...
		return new Promise((resolve, reject)=>{
    		...
    		res.on('end', () => {
				...
				let id = body.search(/id=".+"><h/i);
				const query = body.substr(id+4, 8);;
	...
	sendRequestRandomWord(endpoint){
		const url = BASE_URL + endpoint;
		const headers = 
			{
				'User-Agent':'Diccionario/2 CFNetwork/808.2.16 Darwin/16.3.0',
		...
		return new Promise((resolve, reject)=>{
    		...
    		res.on('end', () => {
				// deletes <sup>1<\/sup>, which was altering a near condition
				body = body.replace(/<sup>\d*<\\\/sup>/g, '');
				let definicion = body.match(/Definición de.+" class/i);
				//const query1 = definicion.substr(definicion+4,-1);
				if(definicion != null){
				definicion = definicion.toString().substring(14, definicion.toString().length-7);
				definicion = definicion.split(", ")[0]
				}
				else{
					console.log("NULOOOOOOOOOOOO")
					definicion = ""
				}
				let id = body.search(/id=".+"><h/i);
				const query = body.substr(id+4, 7);
				const response = definicion;
				resolve(response);
			});
